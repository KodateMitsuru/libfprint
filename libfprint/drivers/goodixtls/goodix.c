...
void goodix_receive_preset_psk_read(FpDevice *dev, guint8 *data, guint16 length,
  ...
  GoodixPresetPskResp *resp_data =
      ((GoodixPresetPskResp *)(sizeof(guint8) + data));
  g_print("length: %d, offset: %d, flags: %d\n", resp_data->length,
          resp_data->offset, resp_data->flags);

  psk_len = GUINT32_FROM_LE(resp_data->length);

  if (length - 9 != psk_len) {
    g_print("Preset Crap failed len: %d, psk_len: %d\n", length,
            psk_len + sizeof(guint8) + sizeof(GoodixPresetPsk));
    g_set_error(&error, G_IO_ERROR, G_IO_ERROR_INVALID_DATA,
                "Invalid preset PSK read reply length: %d", length);
    callback(dev, FALSE, 0x00000000, NULL, 0, cb_info->user_data, error);
    return;
  ...
  callback(dev, TRUE, GUINT32_FROM_LE(resp_data->flags),
           data + sizeof(GoodixPresetPsk) - 3, psk_len, cb_info->user_data,
           NULL);
}
...
void goodix_receive_pack(FpDevice *dev, guint8 *data, guint32 length) {
  ...
  switch (flags) {
    ...
    case GOODIX_FLAGS_TLS_DATA:
      ...
      // GOODIX 5503: Remove first 9 to get valid TLS content
...
void goodix_send_mcu_switch_to_fdt_up_no_reply(FpDevice *dev, guint8 *mode,
                                               guint16 length,
                                               GDestroyNotify free_func,
                                               GoodixDefaultCallback callback,
                                               gpointer user_data) {
...
void goodix_send_mcu_switch_to_sleep_mode(FpDevice *dev, guint8 sleep_time,
                                          GoodixNoneCallback callback,
                                          gpointer user_data) {
  GoodixMcuSwitchToIdleMode payload = {.sleep_time = sleep_time};
  GoodixCallbackInfo *cb_info;
...
void goodix_send_query_mcu_state(FpDevice *dev, guint8 *payload, guint16 length,
                                 GoodixDefaultCallback callback,
                                 gpointer user_data) {
  GoodixCallbackInfo *cb_info;
...
static void on_goodix_tls_read_handshake(FpDevice *dev, guint8 *data,
  ...
  FpiDeviceGoodixTls *self = FPI_DEVICE_GOODIXTLS(fpi_ssm_get_data(user_data));
  FpiDeviceGoodixTlsPrivate *priv =
      fpi_device_goodixtls_get_instance_private(self);
...
static void tls_handshake_done(FpiSsm *ssm, FpDevice *dev, GError *error) {
  if (error)
    fp_dbg("failed to do tls handshake: %s (code: %d)", error->message,
           error->code);
  goodix_send_tls_successfully_established(dev, on_tls_successfully_established,
                                           NULL);
}
...
static void goodix_tls_ready(GoodixTlsServer *server, GError *err,
                             gpointer dev) {
  if (err) {
    fp_err("failed to init tls server: %s, code: %d", err->message, err->code);
    return;
  }
...
void goodix_tls(FpDevice *dev, GoodixNoneCallback callback,
                gpointer user_data) {
